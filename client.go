...
// Package whatsgo implements a client for interacting with the WhatsApp web multidevice API.
package whatsgo
...
type Client struct {
	...
	// Should whatsgo store recently sent messages in the database so that retry receipts can be accepted
...
//		panic(err)
//	}
//	client := whatsgo.NewClient(deviceStore, nil)
