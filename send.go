...
//	cli.SendMessage(context.Background(), targetJID, &waE2E.Message{...}, whatsgo.SendRequestExtra{ID: msgID})
...
// GenerateMessageID generates a random string that can be used as a message ID on WhatsApp.
//
//	msgID := whatsgo.GenerateMessageID()
//	cli.SendMessage(context.Background(), targetJID, &waE2E.Message{...}, whatsgo.SendRequestExtra{ID: msgID})
//
...
//
//	cli.SendMessage(ctx, to, message, whatsgo.SendRequestExtra{...})
//
...
// The built message can be sent normally using Client.SendMessage.
//
//	resp, err := cli.SendMessage(context.Background(), chat, cli.BuildReaction(chat, senderJID, targetMessageID, "ðŸˆï¸")
//
