...
//
//	resp, err := cli.Upload(context.Background(), yourImageBytes, whatsgo.MediaImage)
//	// handle error
//
...
func (cli *Client) UploadReader(ctx context.Context, plaintext io.Reader, tempFile io.ReadWriteSeeker, appInfo MediaType) (resp UploadResponse, err error) {
	...
	iv, cipherKey, macKey, _ := getMediaKeys(resp.MediaKey, appInfo)
	if tempFile == nil {
		tempFile, err = os.CreateTemp("", "whatsgo-upload-*")
		if err != nil {
...
// Example:
//
//	resp, err := cli.UploadNewsletter(context.Background(), yourImageBytes, whatsgo.MediaImage)
//	// handle error
//
//	imageMsg := &waE2E.ImageMessage{
